...
describe("ActionPanel", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(itemsAPI.uploadImage).mockResolvedValue({ success: true, imageLink: "https://example.com/uploaded.jpg" });
  });
  ...
  describe("View Mode (not editing)", () => {
    ...
    it("saves notes when Save button clicked in view mode", async () => {
      ...
      await waitFor(() => {
        expect(vi.mocked(itemsAPI.updateItem)).toHaveBeenCalledWith("test-team-123", "test-item-456", expect.any(Object));
  ...
  describe("Create Mode", () => {
    ...
    it("creates item and navigates when Create clicked with image", async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockSetShowSuccess).toHaveBeenCalledWith(true);
        expect(mockNavigate).toHaveBeenCalledWith("/teams/to-review/test-team-123", { replace: true });
      });
    });
  ...
  describe("Delete Functionality", () => {
    ...
    it("deletes item and navigates when confirmed", async () => {
      ...
      await waitFor(() => {
        expect(vi.mocked(itemsAPI.deleteItem)).toHaveBeenCalledWith("test-team-123", "test-item-456");
  ...
  describe("Status Cascade to Children", () => {
    it("updates all children when parent status changes", async () => {
      ...
      await waitFor(() => {
        expect(vi.mocked(itemsAPI.updateItem)).toHaveBeenCalledWith("test-team-123", "test-item-456", expect.any(Object));
        expect(vi.mocked(itemsAPI.updateItem)).toHaveBeenCalledWith("test-team-123", "child-1", { status: "Found" });
        expect(vi.mocked(itemsAPI.updateItem)).toHaveBeenCalledWith("test-team-123", "child-2", { status: "Found" });
      });
    });
    ...
    it("recursively updates grandchildren", async () => {
          ...
          children: [
            {
              ...
              status: "Incomplete",
              children: [
                { itemId: "grandchild-1", status: "Incomplete", children: [] },
              ],
            },
          ],
        },
      ...
      await waitFor(() => {
        expect(vi.mocked(itemsAPI.updateItem)).toHaveBeenCalledWith("test-team-123", "child-1", { status: "Damaged" });
        expect(vi.mocked(itemsAPI.updateItem)).toHaveBeenCalledWith("test-team-123", "grandchild-1", { status: "Damaged" });
      });
    });
